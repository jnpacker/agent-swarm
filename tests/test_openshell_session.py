"""
TDD tests for swarmer.openshell_client — the OpenShell SDK wrapper.

These tests are RED until ACM-34583 implements swarmer/openshell_client.py.

Validates the full session lifecycle via OpenShell:
  - Provider created from DB credentials (NOT K8s Secrets)
  - SandboxClient.create() called with BYOC image, provider, policy YAML
  - SandboxClient.wait_ready() called after create
  - exec() called for git clone → config inject → AGENTS.md → agent start
  - SandboxClient.delete() called on session stop
  - K8s Secrets (create_session_agent_secret, etc.) are NEVER called
  - PVC operations (ensure_session_pvc, delete_session_pvc) are NEVER called
"""
import sys
import os
import pytest
...
def _require_client():
    if not _HAS_CLIENT:
        pytest.fail(
            "swarmer.openshell_client not implemented — ACM-34583 required. "
...
@pytest.mark.asyncio
async def test_create_provider_does_not_create_k8s_agent_secret(sdk_client, session, workspace_secret):
    """K8s create_session_agent_secret must NOT be called when using OpenShell Providers."""
    _require_client()
    from swarmer import k8s
    with patch.object(oc, "_get_client", return_value=sdk_client):
        with patch.object(k8s, "create_session_agent_secret") as mock_fn:
...
@pytest.mark.asyncio
async def test_create_provider_does_not_create_k8s_pat_secret(sdk_client, session, workspace_secret, github_pat):
    """K8s create_session_pat_secret must NOT be called when using OpenShell Providers."""
    _require_client()
    from swarmer import k8s
    with patch.object(oc, "_get_client", return_value=sdk_client):
        with patch.object(k8s, "create_session_pat_secret") as mock_fn:
            await oc.create_provider(
                session=session,
                workspace_secret=workspace_secret,
...
@pytest.mark.asyncio
async def test_create_sandbox_uses_byoc_image(sdk_client):
    """SandboxClient.create() must be called with the agent BYOC image."""
    _require_client()
    ...
    call_repr = str(sdk_client.create.call_args)
    assert image in call_repr


@pytest.mark.asyncio
async def test_wait_ready_called_after_create(sdk_client):
    """SandboxClient.wait_ready() must be called after sandbox creation."""
    _require_client()
    with patch.object(oc, "_get_client", return_value=sdk_client):
        await oc.create_sandbox(
            image="quay.io/jpacker/opencode:latest",
...
async def test_create_sandbox_does_not_create_pvc(sdk_client):
    """ensure_session_pvc must NOT be called — PVCs are gone in the OpenShell model."""
    _require_client()
...
async def test_git_clone_exec_targets_sandbox_path(sdk_client, session):
    """exec() for git clone must use /sandbox/ as target, not /workspace/."""
    _require_client()
    ...
    # Verify /sandbox/ path appears in exec call, not /workspace/
...
async def test_git_clone_exec_called_per_repo(sdk_client, session):
    """exec() for git clone must be called once per repo attached to the session."""
    _require_client()
    repo2 = MagicMock()
...
async def test_config_write_exec_uses_sandbox_config_path(sdk_client):
    """exec() for config inject must write to /sandbox/.config/, not /workspace/.config/."""
    _require_client()
    sandbox_name = "sandbox-s42-abc1"
...
async def test_agents_md_exec_writes_to_sandbox(sdk_client):
    """exec() for AGENTS.md must target /sandbox/AGENTS.md."""
    _require_client()
    sandbox_name = "sandbox-s42-abc1"
    with patch.object(oc, "_get_client", return_value=sdk_client):
...
async def test_start_agent_exec_called_with_agent_cmd(sdk_client):
    """exec() must start the agent binary after all setup steps."""
    _require_client()
    sandbox_name = "sandbox-s42-abc1"
...
async def test_stop_calls_delete_sandbox(sdk_client):
    """delete_sandbox must call SandboxClient.delete() to clean up the sandbox."""
    _require_client()
    sandbox_name = "sandbox-s42-abc1"
    with patch.object(oc, "_get_client", return_value=sdk_client):
...
async def test_stop_does_not_call_pvc_delete(sdk_client):
    """delete_session_pvc must NOT be called on stop — there are no PVCs."""
    _require_client()
    from swarmer import k8s_session as k8s_sess
    with patch.object(oc, "_get_client", return_value=sdk_client):
...
async def test_stop_does_not_call_cleanup_session_secrets(sdk_client):
    """cleanup_session_secrets must NOT be called — there are no K8s Secrets to clean up."""
    _require_client()
    from swarmer import k8s
    with patch.object(oc, "_get_client", return_value=sdk_client):
        with patch.object(k8s, "cleanup_session_secrets") as mock_clean:
            await oc.delete_sandbox(sandbox_name="sandbox-s42-abc1")
