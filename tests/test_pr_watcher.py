...
class TestPRWatcherSignalRouting(unittest.TestCase):
    ...
    def test_review_comments_match_specific_and_catchall(self):
        from swarmer.pr_watcher import _collect_pr_signals, _match_triggers_for_pr

        pr = self._make_pr()
        signals = _collect_pr_signals(pr, label_events=[])
        ...
        self.assertEqual(matches[0][3], PRAction.HYGIENE)
        self.assertEqual(matches[1][0].id, 17)
        self.assertEqual(matches[1][3], PRAction.FIX)

    def test_any_actionable_maps_new_pr_to_review(self):
        from swarmer.pr_watcher import _collect_pr_signals, _match_triggers_for_pr

        pr = self._make_pr(unresolved_review_comments=0, coderabbit_unresolved_comments=0)
        signals = _collect_pr_signals(pr, label_events=[])
        self.assertIn("new_pr_or_commit", signals)

        sched_catchall = self._make_schedule(44, "any_actionable", "team")
        matches = _match_triggers_for_pr(pr, signals, [(sched_catchall, self._make_session(77))])
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0][3], PRAction.REVIEW)

    def test_action_dedupe_key_includes_schedule_id(self):
        from swarmer.pr_watcher import _action_dedupe_key

        self.assertEqual(_action_dedupe_key(PRAction.HYGIENE, 25), "pr-hygiene@s25")
        self.assertEqual(_action_dedupe_key(PRAction.FIX, 17), "pr-fix@s17")
...
class TestPRWatcherDispatchFlow(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_evaluate_dispatches_one_and_queues_second_same_session(self):
        ...
        from swarmer.pr_watcher import _action_dedupe_key, _evaluate_and_dispatch_prs
        ...
        async with self.session_factory() as db:
            ...
            with patch("swarmer.pr_watcher._fetch_open_prs", new=AsyncMock(return_value=[{"number": 153}])), \
                ...
                hygiene_key = _action_dedupe_key(PRAction.HYGIENE, sched_hygiene.id)
                catchall_key = _action_dedupe_key(PRAction.FIX, sched_catchall.id)
                hygiene_row = await get_action_state(db, "stolostron/agent-swarm", 153, "sha153", hygiene_key)
                catchall_row = await get_action_state(db, "stolostron/agent-swarm", 153, "sha153", catchall_key)
    ...
    async def test_drain_queued_dispatch_launches_next(self):
        ...
        from swarmer.pr_watcher import _action_dedupe_key, _drain_queued_dispatches
        from swarmer.pr_watcher_store import get_action_state, record_dispatch
        ...
        async with self.session_factory() as db:
            ...
            event_ctx = {
                ...
                "pr_number": 153,
                "head_sha": "sha153",
                "action": PRAction.HYGIENE.value,
                "action_key": action_key,
                "matched_condition": "review_comments",
    ...
    async def test_fix_author_pr_can_still_dispatch_review_signal(self):
        ...
        async with self.session_factory() as db:
            ...
            review_key = _action_dedupe_key(PRAction.REVIEW, sched_review.id)
