...
| `claude_preset_plan_variant` | `CLAUDE_PRESET_PLAN_VARIANT` | `` (unset) | Optional reasoning variant (opencode `agent.plan.variant`) applied to the Claude PLAN-role model |
...
| `gemini_preset_plan_variant` | `GEMINI_PRESET_PLAN_VARIANT` | `high` | Reasoning variant (opencode `agent.plan.variant`) applied to the Gemini PLAN-role model |
